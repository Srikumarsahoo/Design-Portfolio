...
export default function ProjectDetailClient({ project, allProjects, authorName }) {
    ...
    <main className="min-h-screen bg-[#f5f5f0]" ref={sectionRef}>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 nav-glass py-4">
        <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
          ...
          <Link href="/" className="text-[17px] font-semibold">{authorName}</Link>
          <a
            href={project.link}
            target="_blank"
            rel="noopener noreferrer"
            className="btn-lime text-[13px] px-4 py-2 flex items-center gap-1"
          >
      ...
      <section className="pt-24 pb-0 hero-gradient">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <h1
            className="animate-item anim-hidden text-[52px] lg:text-[72px] font-bold tracking-tight text-[#0d0d0d] leading-[1.05] mb-4"
            style={{ transitionDelay: '0.1s' }}
          >
            {project.title}
          </h1>
          <p
            className="animate-item anim-hidden text-[18px] text-[#555] max-w-2xl leading-relaxed mb-10"
          ...
          {/* Overview pills */}
          <div
            className="animate-item anim-hidden grid grid-cols-2 md:grid-cols-4 gap-4 mb-10"
            style={{ transitionDelay: '0.3s' }}
          >
            {[
              { label: 'Role',     value: project.overview.role },
              { label: 'Timeline', value: project.overview.timeline },
              { label: 'Tools',    value: project.overview.tools.join(', ') },
          ...
          {/* Cover */}
          <div
            className="animate-item anim-hidden relative w-full aspect-[16/8] rounded-t-3xl overflow-hidden bg-gradient-to-br from-[#d4f76e]/40 to-[#b5f23d]/20"
          ...
          <div className="space-y-8 mt-10">
            {project.process.map((step, i) => (
              ...
              >
                ...
                <div className={`relative aspect-[4/3] rounded-2xl overflow-hidden bg-gradient-to-br from-[#d4f76e]/30 to-[#b5f23d]/10 ${i % 2 === 1 ? 'lg:col-start-1 lg:row-start-1' : ''}`}>
                  {step.image && (
                    <Image
                      src={step.image}
                      alt={step.title}
            ...
            <div className="grid grid-cols-12 grid-rows-2 gap-4 h-[560px]">
              <div className="animate-item anim-hidden col-span-7 row-span-2 relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#c8f55a]/20 to-[#b5f23d]/10 shadow-xl group">
                {project.solution.images?.[0] && (
                  <Image src={project.solution.images[0]} alt="Solution 1" fill
              ...
              <div className="animate-item anim-hidden col-span-5 row-span-1 relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#e0f0c0]/40 to-[#d4f76e]/20 shadow-lg group" style={{ transitionDelay: '0.1s' }}>
                {project.solution.images?.[1] && (
                  <Image src={project.solution.images[1]} alt="Solution 2" fill
                    className="object-cover transition-transform duration-700 group-hover:scale-105"
                    onError={(e) => { e.target.style.display = 'none' }} />
                )}
              </div>
              <div className="animate-item anim-hidden col-span-3 row-span-1 relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#f0f8e0]/60 to-[#c8f55a]/20 shadow-lg group" style={{ transitionDelay: '0.2s' }}>
                {project.solution.images?.[2] && (
                  <Image src={project.solution.images[2]} alt="Solution 3" fill
                    className="object-cover transition-transform duration-700 group-hover:scale-105"
                    onError={(e) => { e.target.style.display = 'none' }} />
                )}
              </div>
              <div className="animate-item anim-hidden col-span-2 row-span-1 relative rounded-3xl overflow-hidden bg-[#b5f23d]/30 shadow-lg flex items-center justify-center" style={{ transitionDelay: '0.3s' }}>
                ...
                <span className="relative z-10 text-[#3a6b00] font-bold text-[13px] text-center px-2">+More</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="mt-10 grid grid-cols-1 lg:grid-cols-3 gap-5 mb-8">
            {project.results.metrics.map((m, i) => (
              <div key={i} className="animate-item anim-hidden card p-8 text-center" style={{ transitionDelay: `${i * 0.1}s` }}>
